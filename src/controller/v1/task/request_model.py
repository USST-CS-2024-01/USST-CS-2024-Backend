...
class CreateTaskRequest(BaseModel):
    ...
    content: str = Field(..., description="任务简介（支持Markdown格式）", max_length=10000, min_length=1)
    ...
    grade_percentage: float = Field(..., description="该部分任务占总分比重，所有任务比重之和不建议超过100%")


class SetTaskSequenceRequest(BaseModel):
    sequences: List[int] = Field(..., description="所有任务的完整排序，需要将所有该班级的任务都列入进去，且不能重复")
